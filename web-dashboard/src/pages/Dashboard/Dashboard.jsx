...
import { HourlyChart, DailyChart, GravedadChart, TipologiaChart, Top5Vias } from './Charts';
...
export default function Dashboard() {
  ...
  return (
    <div className={`min-h-screen lg:h-screen flex flex-col font-sans ${isDark ? 'dark bg-gray-950' : 'bg-slate-100'}`}>
      <Header año={año} setAño={setAño} gravedad={gravedad} setGravedad={setGravedad} mes={mes} setMes={setMes} isDark={isDark} setIsDark={setIsDark} />

      <div className="flex flex-col lg:flex-row flex-1 lg:overflow-hidden">
        <main className="w-full lg:w-[70%] overflow-y-auto p-4 lg:p-6 bg-slate-100 dark:bg-gray-950 space-y-4">

          {/* KPIs */}
          <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 transition-opacity duration-300 ${loading ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
            {kpi && <>
              <KPICard titulo="Total Histórico" valor={kpi.total} tendencia={kpi.t1} icono={<Activity size={20} />} color="bg-blue-600" />
              <KPICard titulo="Total Víctimas" valor={kpi.victimas} tendencia={kpi.t2} icono={<Users size={20} />} color="bg-red-500" />
              <KPICard titulo="Promedio Mensual" valor={kpi.promedio} tendencia={kpi.t3} icono={<BarChart2 size={20} />} color="bg-amber-500" />
            </>}
          </div>
